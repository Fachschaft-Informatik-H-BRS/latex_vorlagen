...
\begin{columns}[T]
    ...
    \begin{column}{0.42\textwidth}
        \centering
        \includegraphics[width=\linewidth,keepaspectratio]{2.0.0.0_content/assets/bilder/smd_ics_chips_green_pcb_schräge_aufnahme.jpg}
    \end{column}
\end{columns}
\end{frame}
...
\begin{columns}[T]
    ...
    \begin{column}{0.42\textwidth}
        \centering
        \includegraphics[width=\linewidth,keepaspectratio]{2.0.0.0_content/assets/bilder/taschenrechner_fertig_eingeschaltet.jpg}
    \end{column}
\end{columns}
\end{frame}
...
\begin{columns}[T]
    \begin{column}{0.58\textwidth}
        \small
        \begin{itemize}\setlength{\itemsep}{3mm}
            \item \textbf{Weichlöten} wird vor allem in der \textbf{Elektronik} eingesetzt, da es bei niedrigeren Temperaturen (unter 450 °C) arbeitet und empfindliche Bauteile schont, die nicht so viel Hitze vertragen, wie es beim Hartlöten der Fall ist, und es ermöglicht eine gute elektrische Verbindung.
...
\begin{columns}[T]
    \begin{column}{0.58\textwidth}
        \small
        \begin{itemize}\setlength{\itemsep}{3mm}
            \item \textbf{Hochtemperaturlöten} wird für spezielle Anwendungen wie \textbf{Hochleistungselektronik} oder \textbf{Hitzebeständigkeit} verwendet.
            \item Wir arbeiten ausschließlich mit \textbf{Weichlöten}, da es für elektronische Bauteile in der größe in der wir arbeiten am besten geeignet ist.
    ...
    \begin{column}{0.42\textwidth}
        \centering
        \includegraphics[width=\linewidth,keepaspectratio]{2.0.0.0_content/assets/bilder/hochtemperaturlöten.jpg}
    \end{column}
\end{columns}
\end{frame}

% \begin{frame}[fragile]{Lötarten}
...
\begin{columns}[T]
    ...
    \begin{column}{0.42\textwidth}
        \centering
        \includegraphics[width=\linewidth,keepaspectratio]{2.0.0.0_content/assets/bilder/spitzen_aktivator.jpg}
    \end{column}
\end{columns}
\end{frame}